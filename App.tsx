...
import { createAlbumPage, createPolaroidImage } from './lib/albumUtils';
import { dataUrlToFile } from './lib/utils';
import Footer from './components/Footer';

const DECADES = ['1920s', '1930s', '1940s', '1950s', '1960s', '1970s', '1980s', '1990s', '2000s'];

// Pre-defined positions for a scattered look on desktop
const POSITIONS = [
    { top: '2%', left: '5%', rotate: -10 },   // 1920s
    { top: '35%', left: '15%', rotate: 8 },  // 1930s
    { top: '5%', left: '30%', rotate: 5 },   // 1940s
    { top: '50%', left: '2%', rotate: -5 },  // 1950s
    { top: '2%', left: '55%', rotate: 12 },  // 1960s
    { top: '45%', left: '40%', rotate: -3 }, // 1970s
    { top: '15%', left: '75%', rotate: 7 },  // 1980s
    { top: '55%', left: '65%', rotate: -9 }, // 1990s
    { top: '40%', left: '80%', rotate: 4 },  // 2000s
];
...
function App() {
    ...
    const handleDownloadIndividualImage = async (decade: string) => {
        ...
        if (image?.status === 'done' && image.url) {
            try {
                const polaroidDataUrl = await createPolaroidImage(image.url, decade);
                const link = document.createElement('a');
                link.href = polaroidDataUrl;
                link.download = `through-time-${decade}.jpg`;
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
            } catch (error) {
                console.error("Failed to create or download polaroid image:", error);
                alert("Sorry, there was an error creating your polaroid. Please try again.");
            }
        }
    };
    ...
    const handleShareIndividualImage = async (decade: string) => {
        ...
        if (image?.status === 'done' && image.url) {
            try {
                const polaroidDataUrl = await createPolaroidImage(image.url, decade);
                const file = await dataUrlToFile(polaroidDataUrl, `through-time-${decade}.jpg`);
                
                if (file && navigator.canShare({ files: [file] })) {
                    await navigator.share({
                        title: `Me in the ${decade} - Through Time`,
                        text: `Check out my photo from the ${decade}, generated with Through Time on Google AI Studio!`,
                        files: [file],
                    });
                } else {
                    alert("Sorry, your browser can't share this image file.");
                }
            } catch (error) {
                if ((error as Error).name !== 'AbortError') {
                    console.error("Failed to share polaroid image:", error);
                    alert("Sorry, there was an error preparing your polaroid for sharing.");
                }
            }
        }
    };
    ...
    const handleDownloadAlbum = async () => {
        ...
        try {
            ...
            link.download = 'through-time-album.jpg';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
    ...
    const handleShareAlbum = async () => {
        ...
        try {
            ...
            if (Object.keys(imageData).length < DECADES.length) {
                alert("Please wait for all images to finish generating before sharing the album.");
                return;
            }

            const albumDataUrl = await createAlbumPage(imageData);
            const file = await dataUrlToFile(albumDataUrl, 'through-time-album.jpg');

            if (file && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    title: 'My Through Time Album',
                    text: 'Check out my journey through the decades, generated with Through Time on Google AI Studio!',
                    files: [file],
                });
            } else {
                alert("Sorry, your browser can't share this album file.");
            }
        } catch (error) {
            if ((error as Error).name !== 'AbortError') {
        ...
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 pb-24 overflow-hidden relative">
            ...
            <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
                <div className="text-center mb-10">
                    <h1 className="text-6xl md:text-8xl font-caveat font-bold text-neutral-100">Through Time</h1>
                                ...
                                <div className="flex flex-col sm:flex-row items-center gap-4">
                                    {navigator.share && (
                                        <button 
                                            ...
                                            disabled={isSharing} 
                                            className={`${secondaryButtonClasses} disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center ${isSharing ? 'animate-pulse' : ''}`}
                                        >
                                            {isSharing ? (
                                                <>
                                    ...
                                    <button 
                                        ...
                                        className={`${primaryButtonClasses} disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center ${isDownloading ? 'animate-pulse' : ''}`}
                                    >
                                        {isDownloading ? (
                                            <>
                                                <svg aria-hidden="true" className="animate-spin -ml-1 mr-3 h-5 w-5 text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                                </svg>
                                                Creating Album...
